...
const _require = require('./DependencyGraph/ModuleResolution');
  const ModuleResolver = _require.ModuleResolver;

const _require2 = require('events');
  const EventEmitter = _require2.EventEmitter;

const _require3 = require('metro-core');
  const _require3$Logger = _require3.Logger;
  const createActionStartEntry = _require3$Logger.createActionStartEntry;
  const createActionEndEntry = _require3$Logger.createActionEndEntry;
  const log = _require3$Logger.log;
...
class DependencyGraph extends EventEmitter {
  constructor(_ref) {
    const config = _ref.config;
      let haste = _ref.haste;
      let initialHasteFS = _ref.initialHasteFS;
      let initialModuleMap = _ref.initialModuleMap;
    super();

    _defineProperty(this, '_doesFileExist', (filePath) => this._hasteFS.exists(filePath));
  ...
  static load(config) {
    return _asyncToGenerator(function* () {
      ...
      const _ref2 = yield haste.build();
        const hasteFS = _ref2.hasteFS;
        const moduleMap = _ref2.moduleMap;
  ...
  _onHasteChange(_ref3) {
    const eventsQueue = _ref3.eventsQueue;
      let hasteFS = _ref3.hasteFS;
      let moduleMap = _ref3.moduleMap;
  ...
  _createModuleResolver() {
    this._moduleResolver = new ModuleResolver({
      ...
      resolveAsset: (dirPath, assetName, platform) => this._assetResolutionCache.resolve(dirPath, assetName, platform),
  ...
  getSha1(filename) {
    ...
    const containerName =      splitIndex !== -1 ? filename.slice(0, splitIndex + 4) : filename; // TODO Calling realpath allows us to get a hash for a given path even when
